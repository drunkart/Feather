...
coinpunk.controllers.Tx.prototype.create = function() {
  ...
  if(address == '')
    ...
    try {
      ...
      errors.push('The provided feathercoin address is not valid.');
    }
  }
  ...
  for(var i=0; i<myAddresses.length;i++) {
    if(myAddresses[i].address == address)
      errors.push('You cannot send to your own feathercoin wallet.');
  }
  ...
  if(amount == '' || parseFloat(amount) == 0) {
    ...
    errors.push('Cannot spend more feathercoins than you currently have.');
  }
...
coinpunk.controllers.Tx.prototype.calculateFee = function() {
  ...
  $('#fee').text(coinpunk.wallet.calculateFee(amount, address, changeAddress)+' FTC');
