...
DB.prototype = {
  ...
  set: function(serverKey, address, payload, callback) {
    ...
    this.redis.hgetall(serverKey, function(err, res) {
      ...
      self.redis.hmset(serverKey, address, payload, callback);
    });
  ...
  emailtoFTC: function(email, callback) {
      ...
      this.redis.keys('*', function(err, serverKeys) {
        ...
        for(var k=0;k<serverKeys.length;k++) {
            ...
            self.redis.hmget(serverKey, 'email', function(err, res) {
                if(err) {
                    return callback(err)
                }
                if(res.toString().toLowerCase() == email) {
                    self.redis.hgetall(serverKey, function(error, response) {
                        console.log(res.address)
                        if (error) {
                            return callback(error)
                        } else {
                            return callback(undefined, response.address)
                        }
                    })
                }
            });
