    ...
    conn.on('data', function(message) {
      ...
      if(req.method == 'listUnspent') {
        that.listUnspent(req.addresses, function(err, unspent) {
          console.log("From listUnspent req")
          console.log(unspent)
          if(err)
...
/*Get FTC of the provided email*/
WalletController.prototype._emailtoFTC = function (req, res) {
    var email = req.query.email

    this.db.emailtoFTC(email, function(error, address) {
        res.send({address: address, error: error})
    })
}
...
WalletController.prototype.saveWallet = function (req, res) {
  ...
  that.db.set(req.body.serverKey, req.body.address, req.body.payload, function(err, data) {
    if(err) {
      if(err == 'outOfSync') {
