...
OnenameClient.prototype._postRequest = function(url, payload, callback) {
    request({
        url: url,
        method: 'POST',
        json: payload,
...
OnenameController.prototype._create = function(req, res) {
	console.log('passname : ' + req.body.email + ' recipient_address' + req.body.address)
	//registrationPayload = {'passname': req.body.email, 'recipient_address': req.body.address}
	this._registerUser('fredwilson', 'N6zdUCKq1gJaps76gagBbC5Vc6xBxMdvHc' , function(err, data) {
		if(data) {
			console.log("Logging from _create : " + data)
