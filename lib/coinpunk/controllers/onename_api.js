...
OnenameController.prototype._getftc = function(req, res) {
	var usernames = [req.query.email]
	//var usernames = ['fredwilson']
	this._getUsers(usernames, function(err, users) {
        if (err !== null) {
            return res.send({error: err})
        } else if (hasprop(users, usernames[0] + '.profile')) {
			return res.send({ftcAddress: users[usernames[0]].profile.bitcoin.address})
		}
	})
};

OnenameController.prototype._create = function(req, res) {
	this._registerUser(req.body.email, req.body.address , function(data) {
		if(data) {
